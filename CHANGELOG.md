...
- Bumped resume plugin to v0.1.1 after a caption-sourced fidelity review of the judge: Gate 2 now tests the transcript's actual recruiter criterion — can they find the work fast — with concrete findability checks instead of restating the thesis; the funnel step reads supplied outcomes for noise versus pattern by version, role family, and source, and the decision asks whether a pattern reads as the document or as the market; vendor scorecards are admissible as a parse check at Gate 1 and nowhere else, matching the transcript's permitted use; "Terraform" restored where Whisper heard "platform"; the skill wall separated from the theatre list and hidden text restored to the Gate 1 inventory; "Make it readable, targeted, defensible" and the "blaming a PDF" rationale added; provenance tags corrected where the Part 2 [T] blanket covered judge glosses. `review` SKILL.md no longer contradicts the asset on unanswered questions and names the standard's source in its trigger description.
