...
- **Big-picture event exploration (EventStorming, Big Picture style):** Alberto Brandolini describes EventStorming as a workshop format for quickly exploring complex business domains, bringing question-askers and domain experts together to build a model collaboratively; the original description claims it can produce a comprehensive model of a business flow in hours rather than weeks. The mechanics emphasise starting from domain events placed on a timeline and using the workshop to reveal subdomains/bounded contexts, personas, and even key acceptance tests when ambiguities arise. turn25view0  
