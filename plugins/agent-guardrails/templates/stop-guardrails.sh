...
if echo "$message" | grep -qiE '((shall|should|can|could|may) (I|we)\b|(want|like|need|ready for) me to\b|(would|do) you (want|like|prefer|rather|think|need|mind)\b|would you\b.*\bor\b|your (call|choice|decision|preference)|up to you|let me know|(does|how does) (this|that|it) look|(sound|look) good|((any|some|no)thing|what) else (you|to|would|I|we|can)|is there anything else|what would you like|which (would|do) you|(there are|here are) (a few|some|several|multiple) (approaches|options|ways|alternatives|choices)|happy to .*(either|whichever|whatever)|or (should|do|would|can|could) (I|we|you)\b|what.*feels right|which.*feels right|how about (we|I)\b|\b(what|which|who|whom|whose|where|when|why|how)\b[^.!:?]*\?)'; then
  blocked+=("**no-preference-asking**: Do not ask for preference, seek approval, or defer decisions. The detection uses structural patterns — modal+I/we, want-me-to, do-you-X, your-decision-noun, approval-seeking, option-presenting — not a phrase list. If the grammatical structure of your sentence is asking the user to make a decision you should make yourself, it will be caught. Only ask the user if you genuinely cannot determine the best choice after thorough investigation, and explain exactly why.")
fi
