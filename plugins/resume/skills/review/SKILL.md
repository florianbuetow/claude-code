...
Read `../../references/cv-judge.md` in full before anything else. It defines the two personas and their binding traits, the transcript-derived standard, the ten-step procedure, the calibration anchors, the output formats, and the Always / Never rules. Follow it verbatim. Do not summarise it, shorten it, or substitute your own rubric.
...
3. **Application history**, if the user has it — versions sent, roles, sources, referral state, outcomes. Optional. If absent, say so once and continue; it only feeds Step 8 and the Track tier of the fix list. If `resume-tracking.md` from `/resume:build` exists in the working directory, read it.
...
- Gate 2 (recruiter): does the document make the next decision easy?
...
- Structure: skills placement, role fit, funnel (only if history was supplied).
- Decide: a prediction with its reasoning and remaining unknowns. Not a verdict on the person, not a score.
...
- Never emit an ATS-style score or a single overall number. Per-line findings only.
- Never skip Phase A's stop. The judge does not score a résumé on unanswered questions.
